...
class CacheEngineVMM:
    ...
    def __init__(
        ...
        if self.max_seq_len % self.block_size != 0:
            self.max_seq_len = ((self.max_seq_len // self.block_size + 1)
                                * self.block_size)
            logger.warning("self.max_seq_len mod self.block_size != 0,round up max_seq_len to %d", self.max_seq_len)
        ...
        # sequence_buffer_bytes_size is the number of bytes for each sequence
        self.cache_space_size = self.max_batch_size * self.sequence_buffer_size
        # Maximum memory for all batched requests together for each layer.
        self.cache_sapce_bytes_size = self.cache_space_size * self.dtype_size

        # block_bytes_size == 2M
        assert (
                   self.cache_sapce_bytes_size) % self.block_bytes_size == 0, "cache_sapce_bytes_size must be divisible by block_bytes_size"
    ...
    def alloc_seqs(self, allocated_block_counts: Dict[int, int]):
        ...
        for buffer_id, num_blocks in allocated_block_counts.items():
            ...
            if num_blocks > 0:
                allocated_blocks = self.allocated_block_counts[buffer_id]
                offset = (start_offset + allocated_blocks * self.block_bytes_size)
