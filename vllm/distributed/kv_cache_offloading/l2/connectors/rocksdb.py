...
class RocksDBConnector(Connector[bytes, V]):
    ...
    @classmethod
    def from_envs(cls, retention_type: KVCacheRetentionType,
                  conn_id: str) -> "RocksDBConnector":
        ...
        if retention_type == KVCacheRetentionType.CACHING:
            # use TTL to manage the life cycle of the data in the KV cache
            access_type = access_type.with_ttl(
                envs.VLLM_KV_CACHE_OL_ROCKSDB_TTL_S)
        return cls(root, opts, access_type, retention_type)
