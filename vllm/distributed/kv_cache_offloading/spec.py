...
class KVCacheBlockSpec:
    ...
    def __post_init__(self):
        ...
        if self.block_layout == KVCacheBlockLayout.NCLD:
            ...
            self.block_shape_token_dim: int = 2

    def _get_block_shape(self) -> Tuple[int, ...]:
        if self.block_layout == KVCacheBlockLayout.NCLD:
            return (self.block_ntokens, 2, len(self.tensor_spec.layers),
                    len(self.tensor_spec.heads), self.tensor_spec.head_size)
        elif self.block_layout == KVCacheBlockLayout.LCND:
            return (len(self.tensor_spec.layers), 2, self.block_ntokens,
                    len(self.tensor_spec.heads), self.tensor_spec.head_size)
