...
class MemoryRegion(RefCountedObj):
    ...
    @staticmethod
    def is_appendable(src: 'MemoryRegion', dst: 'MemoryRegion') -> bool:
        """
    ...
    def split(mr: 'MemoryRegion',
        ...
        with mr._lock:
            use_finalizer = mr._use_finalizer
            return tuple(
                MemoryRegion(mr.allocator, mr.addr +
                             offset, split_size, use_finalizer)
                for offset in range(0, mr.length, split_size))

    @staticmethod
    def expand(mr: 'MemoryRegion', expand_size: int) -> 'MemoryRegion':
    ...
    def to_tensor(self) -> torch.Tensor:
        ...
        return self.allocator._buffer[self.addr:self.addr + self.length]

    @staticmethod
    def to_tensors(mrs: Iterable['MemoryRegion']) -> Iterable[torch.Tensor]:
        ...
        addr = mrs[0].addr
        length = mrs[0].length
        allocator = mrs[0].allocator
        for i in range(1, len(mrs)):
            if addr + length == mrs[i].addr:
                # merge
                length += mrs[i].length
            else:
                # find a bubble
                tensors.append(allocator._buffer[addr:addr + length])
                addr = mrs[i].addr
                length = mrs[i].length
        tensors.append(allocator._buffer[addr:addr + length])
        return tensors
...
class TensorPoolAllocator(ABC):
    ...
    def __init__(
        ...
        init_mr: MemoryRegion = MemoryRegion(self,
                                             addr=0,
                                             len=self._buffer.numel(),
                                             use_finalizer=False)
    ...
    def alloc(self, size: int) -> Status[MemoryRegion]:
        ...
        with self._lock:
            ...
            if target_mr_len > size:
                left_over_mr = MemoryRegion(
                    self,
                    addr=target_mr.addr + size,
                    len=target_mr.length - size,
                    use_finalizer=False,
                )
    ...
    def _finalize_mr(self, addr: int, length: int) -> None:
        with self._lock:
            mr = MemoryRegion(self, addr, length, use_finalizer=False)
            ...
            if MemoryRegion.is_appendable(curr, prev):
                ...
                # Append curr to prev
                curr = MemoryRegion.expand(prev, curr.length)

            # curr = prev + mr if append happened
            # 2. append next to curr if possible
            if MemoryRegion.is_appendable(next, curr):
                ...
                # Append next to curr
                curr = MemoryRegion.expand(curr, next.length)
