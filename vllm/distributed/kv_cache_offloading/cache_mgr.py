...
from .config import KVCacheConfig, KVCacheRetentionType
...
class KVCacheManager(ABC):
    ...
    def acquire(
        ...
        tokens: Iterable[int],
    ) -> Status[Tuple[int, Iterable[KVCacheHandle | Tuple[KVCacheHandle,
                                                          KVCacheHandle]]]]:
    ...
    def get(
        ...
        prefix: Iterable[int] | None,
        tokens: Iterable[int],
    ) -> Status[Tuple[int, torch.Tensor | Tuple[torch.Tensor, torch.Tensor]]]:
        ...
        Args:
            ...
            tokens: The tokens of the kv cache. E.g., [4, 5, 6, 7]
            kv_tensors:
                The kv tensors to store the fetched kv tensors.
                
                The layout of kv_tensors must match the layout of the kv cache service.

                For example, if the layout is NLD, then:
                The k, v tensors for i-th token at the j-th layer is: kv_tensors[i][j]
    ...
    def put(
        ...
        Args:
            ...
            tokens: The tokens of the kv cache. E.g., [4, 5, 6, 7]
            kv_tensors:
                The kv tensors to update the kv cache.
...
class BaseKVCacheManager(KVCacheManager):
    ...
    def __init__(self, config: KVCacheConfig) -> None:
        ...
        if self.config.retention_type == KVCacheRetentionType.STORE:
            raise NotImplementedError(
                "STORE retention type is not supported yet.")
        ...
        if len(envs.VLLM_KV_CACHE_OL_L2_CACHE_BACKEND) > 0:
            ...
            self._l2_cache = L2Cache(
                ...
                retention_type=self.config.retention_type,
    ...
    def feature(self) -> KVCacheFeature:
        ...
        #     return KVCacheFeature(zero_copy=True)
        return KVCacheFeature(zero_copy=False)
    ...
    def acquire(
        ...
        return Status(value=(len(status.value) * self.block_ntokens,
                             BaseKVCacheHandle(status.value)))
    ...
    def _get_impl(
        ...
        zero_copy: bool = False,
                         ...
                         | MemoryRegion | Tuple[MemoryRegion, MemoryRegion]]]:
        """Get kv tensors from the kv cache service.
        ...
        Returns:
            The kv tensors / cache handles corresponding to the tokens.
        """
    ...
    def _merge_kv_tensors(
        self,
        *args: torch.Tensor | Tuple[torch.Tensor, torch.Tensor] | MemoryRegion
        | Tuple[MemoryRegion, MemoryRegion],
        ...
        for mr_or_tensor in args:
            if isinstance(mr_or_tensor, MemoryRegion):
                tensors_or_tuples.append(mr_or_tensor.to_tensor().view(
                    self.block_dtype).view(self.block_shape))
            elif isinstance(mr_or_tensor, Tuple) and isinstance(
                    mr_or_tensor[0], MemoryRegion):
                tensors_or_tuples.append(
                    (mr_or_tensor[0].to_tensor().view(self.block_dtype).view(
                        self.block_shape), mr_or_tensor[1].to_tensor().view(
                            self.block_dtype).view(self.block_shape)))
            else:
        ...
        if isinstance(tensors_or_tuples[0], tuple):
            self._release_mrs(args)
            return (
                torch.cat([tup[0] for tup in tensors_or_tuples], dim=0),
                torch.cat([tup[1] for tup in tensors_or_tuples], dim=0),
            )

        self._release_mrs(args)
        return torch.cat(tensors_or_tuples, dim=0)

    def _release_mrs(
        self, mr_or_tensors: Iterable[torch.Tensor
                                      | Tuple[torch.Tensor, torch.Tensor]
                                      | MemoryRegion
                                      | Tuple[MemoryRegion, MemoryRegion]]
    ) -> None:
        for mr_or_tensor in mr_or_tensors:
            if isinstance(mr_or_tensor, MemoryRegion):
                mr_or_tensor.ref_down()
            elif isinstance(mr_or_tensor, Tuple) and isinstance(
                    mr_or_tensor[0], KVCacheHandle):
                mr_or_tensor[0].ref_down()
                mr_or_tensor[1].ref_down()
...
class GroupAwareKVCacheManager(BaseKVCacheManager):
    ...
    def __init__(self, config: KVCacheConfig,
        ...
        self.chunk_size = envs.VLLM_KV_CACHE_OL_CHUNK_SIZE

        assert self.chunk_size % self.block_ntokens == 0, (
            f"chunk_size must be divisible by block_ntokens, "
            f"but got chunk_size={self.chunk_size} and block_ntokens={self.block_ntokens}"
        )
        if self.chunk_size < 4 * self.block_ntokens:
            logger.warning(
                f"chunk_size is too small, using {4 * self.block_ntokens} instead"
            )
            self.chunk_size = 4 * self.block_ntokens

        self._chunk_size_nblocks = self.chunk_size // self.block_ntokens
    ...
    def get(
        ...
        if not status.is_ok():
            return status
        return Status(value=(status.value[0],
                             self._merge_kv_tensors(*status.value[1])))
    ...
    def acquire(
        ...
        prefix: Iterable[int] | None,
        tokens: Iterable[int],
    ) -> Status[Iterable[KVCacheHandle | Tuple[KVCacheHandle, KVCacheHandle]]]:
        """(Optional) Acquire cache handle of the kv tensors for the given prefix
        and tokens. Only the kv cache services supporting zero-copy need to implement
        this method.
        ...
        if not status.is_ok():
            return status
        handle = BaseKVCacheHandle(status.value[1])
    ...
    def _get_impl(
        ...
        zero_copy: bool = False,
                         ...
                         | KVCacheHandle | Tuple[KVCacheHandle,
                                                 KVCacheHandle]]]:
        ...
        start = 0
        results = []
        for chunk_prefix, chunk_tokens in self._cache_chunk_keys(
                prefix, tokens):
    ...
    def _cache_chunk_keys(
        ...
        Returns:
            The cache chunk keys of the kv tensors.
        """
        cache_key = [] if prefix is None else [x for x in prefix]
        chunk_tokens = split_list(tokens, self.chunk_size)

        for i in range(len(chunk_tokens)):
            yield (tuple(cache_key), tuple(chunk_tokens[i]))
