...
MS_BUCKETS = [
    1, 2, 3, 5, 8, 10, 20, 30, 50, 80, 100, 200, 300, 500, 800, 1000, 2000,
    3000
]
...
class BaseMetricsExporter(MetricsExporter):
    ...
    def _export_gauge(self, gauge, labels: Dict[str, str],
                      data: int | float) -> None:
...
class OpMetrics(Metrics):
    ...
    op_breadowns: List[Breakdowns] = None
    ...
    def summary(self) -> str:
        ...
        avg_fetched_tokens = (total_fetched_tokens /
...
class OpMetricsExporter(BaseMetricsExporter):
    ...
    def _init_exporter_fields(self) -> None:
        ...
        self.histogram_iteration_fetched_tokens = self._histogram_cls(
            ...
            documentation=("Histogram of number of fetched tokens "
                           "per iteration."),
            labelnames=self._labelnames,
            buckets=TOKEN_BUCKETS)
    ...
    def export(self, labels: Dict[str, str], metrics: OpMetrics) -> None:
        if metrics.num_ops <= 0:
            return

        labels = labels.copy()
        labels[self.OP_TYPE_LABELNAME] = metrics._op.name.lower()
        assert set(labels.keys()) == set(self._labelnames), f"Labels " \
            f"{set(labels.keys())} do not match {self._labelnames}"

        self._export_counter(self.counter_num_ops, labels,
                             len(metrics.num_prefixes))
        self._export_counter_set(self.counter_num_errors, labels,
        ...
        self._export_histogram(self.histogram_iteration_tokens, labels,
                               metrics.num_tokens)
        if metrics._is_get():
            self._export_histogram(self.histogram_iteration_fetched_tokens,
                                   labels, metrics.num_fetched_tokens)
...
class UsageMetrics(Metrics):
    """Usage metrics."""
    resource: MetricRecorder.Resource = None
    capacity: int = 0
    used: int = 0
...
class UsageMetricsExporter(BaseMetricsExporter):
    ...
    def _init_exporter_fields(self) -> None:
        self.gauge_used = self._gauge_cls(name=f"{self._prefix}used",
                                          documentation="Used resource.",
                                          labelnames=self._labelnames)
        self.gauge_capacity = self._gauge_cls(name=f"{self._prefix}capacity",
                                              documentation="Capacity.",
                                              labelnames=self._labelnames)

    def export(self, labels: Dict[str, str], metrics: UsageMetrics) -> None:
...
class BaseCacheMetrics(Metrics, MetricRecorder):
    ...
    put_metrics: OpMetrics = None
    get_metrics: OpMetrics = None
    exists_metrics: OpMetrics = None
    ...
    def record(
        ...
        if op is MetricRecorder.OP.PUT:
            self.put_metrics.add(num_prefix, num_tokens, status, lat_ms,
                                 breakdowns)
        elif op is MetricRecorder.OP.GET:
            self.get_metrics.add(num_prefix, num_tokens, status, lat_ms,
                                 breakdowns)
            self._agg_hit_miss(num_tokens, status)
        elif op is MetricRecorder.OP.EXISTS:
            self.exists_metrics.add(num_prefix, num_tokens, status, lat_ms,
                                    breakdowns)
        else:
    ...
    def summary(self) -> str:
        ...
        if self.get_metrics.num_ops > 0:
            summary += ", " if len(summary) > 0 else ""
            hit_rate = (self.num_tokens_hit * 100 /
                        (self.num_tokens_hit + self.num_tokens_miss))
            summary += f"Num. of hit: {self.num_tokens_hit}, " \
...
class BaseCacheMetricsExporter(BaseMetricsExporter):
    ...
    def export(self, labels: Dict[str, str],
               metrics: BaseCacheMetrics) -> None:
        labels = labels.copy()
...
class L1CacheMetrics(BaseCacheMetrics):
    ...
    acquire_metrics: OpMetrics = None
    eviction_policy_usage_metrics: UsageMetrics = None
    allocator_usage_metrics: UsageMetrics = None
    ...
    def record(
        ...
        if op is MetricRecorder.OP.ACQUIRE:
            self.acquire_metrics.add(num_prefix, num_tokens, status, lat_ms,
                                     breakdowns)
            self._agg_hit_miss(num_tokens, status)
        else:
            super().record(op, num_prefix, num_tokens, status, lat_ms,
                           breakdowns)
...
class KVCacheMetricsExporter(BaseCacheMetricsExporter):
    ...
    def export(self, labels: Dict[str, str], metrics: KVCacheMetrics) -> None:
...
class MeasurableBase:
    ...
    def __init__(
        self,
        recorder: BaseCacheMetrics,
    ):
        self._recorder = recorder
        self._enable_time_measurement = (recorder is not None
                                         and recorder.time_measurement_enabled)
        ...
        def measure_wrap(func):
            ...
            async def async_wrapper(
                ...
                **kwargs,
                ...
                with cpu_perf_timer(
                        self._enable_time_measurement) as get_lat_ms:
                    status = await func(self, prefix, tokens, *args, **kwargs)
                if self._recorder:
                    self._recorder.record(op,
                                          0 if prefix is None else len(prefix),
                                          len(tokens), status, get_lat_ms())
                return status
