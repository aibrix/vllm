...
def test_basic(policy):
    ...
    for i in range(capacity):
        ...
        if policy.name == "S3FIFO":
            # for S3FIFO, we need to get the data to trigger promotion to main fifo
            # when eviction happens on small fifo
...
def test_on_hot_access_callback(policy):
    ...
    for i in range(capacity):
        policy.put(i, torch.tensor([1, 2, 3, i]))
        if i % 3 == 0:
            # get multiple times to ensure no duplicate calls to on_hot_access callback
            for _ in range(5):
                assert torch.equal(
