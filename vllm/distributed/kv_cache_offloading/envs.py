...
kv_cache_ol_environment_variables: Dict[str, Callable[[], Any]] = {
    ...
    lambda: tuple(
        map(
            ...
            map(
                str.strip,
                os.getenv("AIBRIX_KV_CACHE_OL_DOUBLE_GET_THRESHOLD", "4,0.1").
                split(","),
            ),
            (int, float),
        )),
    "AIBRIX_KV_CACHE_OL_CHUNK_SIZE":
    lambda: int(os.getenv("AIBRIX_KV_CACHE_OL_CHUNK_SIZE", "512")),
    "AIBRIX_KV_CACHE_OL_METRICS_ENABLED":
    lambda:
    (os.getenv("AIBRIX_KV_CACHE_OL_METRICS_ENABLED", "1").strip().lower() in
     ("1", "true")),
    "AIBRIX_KV_CACHE_OL_TIME_MEASUREMENT_ENABLED":
    lambda: (os.getenv("AIBRIX_KV_CACHE_OL_TIME_MEASUREMENT_ENABLED", "1").
             strip().lower() in ("1", "true")),
    "AIBRIX_KV_CACHE_OL_BREAKDOWN_MEASUREMENT_ENABLED":
    lambda: (os.getenv("AIBRIX_KV_CACHE_OL_BREAKDOWN_MEASUREMENT_ENABLED", "1")
             .strip().lower() in ("1", "true")),
    # ================== L1Cache Env Vars ==================
    "AIBRIX_KV_CACHE_OL_L1_CACHE_ENABLED":
    lambda:
    (os.getenv("AIBRIX_KV_CACHE_OL_L1_CACHE_ENABLED", "1").strip().lower() in
     ("1", "true")),
    "AIBRIX_KV_CACHE_OL_L1_CACHE_EVICTION_POLICY":
    lambda: (os.getenv("AIBRIX_KV_CACHE_OL_L1_CACHE_EVICTION_POLICY", "S3FIFO")
             .strip().upper()),
    "AIBRIX_KV_CACHE_OL_L1_CACHE_CAPACITY_GB":
    lambda: float(os.getenv("AIBRIX_KV_CACHE_OL_L1_CACHE_CAPACITY_GB", "10")),
    "AIBRIX_KV_CACHE_OL_DEVICE":
    lambda: (os.getenv("AIBRIX_KV_CACHE_OL_DEVICE", "cpu").strip().lower()),
    "AIBRIX_KV_CACHE_OL_L1_CACHE_EVICT_SIZE":
    lambda: int(os.getenv("AIBRIX_KV_CACHE_OL_L1_CACHE_EVICT_SIZE", "16")),
    # ================== S3FIFO Env Vars ==================
    # Promotion threshold of small fifo to main fifo
    "AIBRIX_KV_CACHE_OL_S3FIFO_SMALL_TO_MAIN_PROMO_THRESHOLD":
    lambda: int(
        os.getenv("AIBRIX_KV_CACHE_OL_S3FIFO_SMALL_TO_MAIN_PROMO_THRESHOLD",
                  "1")),
    ...
    lambda: (os.getenv("AIBRIX_KV_CACHE_OL_L2_CACHE_NAMESPACE", "aibrix").
             strip().lower()),
    "AIBRIX_KV_CACHE_OL_L2_CACHE_COMPRESSION":
    lambda:
    (os.getenv("AIBRIX_KV_CACHE_OL_L2_CACHE_COMPRESSION", "").strip().upper()),
    "AIBRIX_KV_CACHE_OL_L2_CACHE_OP_BATCH":
    lambda: int(os.getenv("AIBRIX_KV_CACHE_OL_L2_CACHE_OP_BATCH", "32")),
    ...
    lambda: (os.getenv("AIBRIX_KV_CACHE_OL_L2_CACHE_INGESTION_TYPE", "HOT").
             strip().upper()),
    "AIBRIX_KV_CACHE_OL_L2_CACHE_INGESTION_MAX_INFLIGHT_TOKENS":
    lambda: int(
        os.getenv("AIBRIX_KV_CACHE_OL_L2_CACHE_INGESTION_MAX_INFLIGHT_TOKENS",
                  "0")),
    "AIBRIX_KV_CACHE_OL_L2_CACHE_NUM_ASYNC_WORKERS":
    lambda: int(os.getenv("AIBRIX_KV_CACHE_OL_L2_CACHE_NUM_ASYNC_WORKERS", "8")
                ),
    # ================== Mock Connector Env Vars ==================
    "AIBRIX_KV_CACHE_OL_MOCK_USE_RDMA":
    lambda:
    (os.getenv("AIBRIX_KV_CACHE_OL_MOCK_USE_RDMA", "0").strip().lower() in
     ("1", "true")),
    "AIBRIX_KV_CACHE_OL_MOCK_USE_MPUT_MGET":
    lambda:
    (os.getenv("AIBRIX_KV_CACHE_OL_MOCK_USE_MPUT_MGET", "0").strip().lower() in
     ("1", "true")),
    # ================== RocksDB Env Vars ==================
    ...
    lambda: int(os.getenv("AIBRIX_KV_CACHE_OL_ROCKSDB_TTL_S", "600")),
    "AIBRIX_KV_CACHE_OL_ROCKSDB_WRITE_BUFFER_SIZE":
    lambda: int(
        os.getenv("AIBRIX_KV_CACHE_OL_ROCKSDB_WRITE_BUFFER_SIZE",
                  f"{64 * 1024 * 1024}")),
    ...
    lambda: (os.getenv("AIBRIX_KV_CACHE_OL_INFINISTORE_HOST_ADDR", "127.0.0.1")
             .strip()),
    ...
    lambda: (os.getenv("AIBRIX_KV_CACHE_OL_INFINISTORE_CONNECTION_TYPE", "RDMA"
                       ).strip().upper()),
    "AIBRIX_KV_CACHE_OL_INFINISTORE_IB_PORT":
    lambda: int(os.getenv("AIBRIX_KV_CACHE_OL_INFINISTORE_IB_PORT", "12345")),
    "AIBRIX_KV_CACHE_OL_INFINISTORE_LINK_TYPE":
    lambda: (os.getenv("AIBRIX_KV_CACHE_OL_INFINISTORE_LINK_TYPE", "Ethernet").
             strip()),
    "AIBRIX_KV_CACHE_OL_INFINISTORE_DEV_NAME":
    lambda:
    (os.getenv("AIBRIX_KV_CACHE_OL_INFINISTORE_DEV_NAME", "mlx5_0").strip()),
    "AIBRIX_KV_CACHE_OL_INFINISTORE_USE_GDR":
    lambda: (os.getenv("AIBRIX_KV_CACHE_OL_INFINISTORE_USE_GDR", "1").strip().
             lower() in ("1", "true")),
    # ================== HPKV Env Vars ==================
    ...
    lambda: int(os.getenv("AIBRIX_KV_CACHE_OL_HPKV_REMOTE_PORT", "12346")),
    ...
    lambda: int(os.getenv("AIBRIX_KV_CACHE_OL_HPKV_LOCAL_PORT", "12345")),
    "AIBRIX_KV_CACHE_OL_HPKV_USE_GDR":
    lambda:
    (os.getenv("AIBRIX_KV_CACHE_OL_HPKV_USE_GDR", "1").strip().lower() in
     ("1", "true")),
}
