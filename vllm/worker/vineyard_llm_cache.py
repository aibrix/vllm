...
class VineyardLLMCache:
    ...
    def prefetch_seq_kv_caches(
        ...
        if seq_group_metadata is not None:
            ...
            logger.info(f"broadcast_object_list seq_id {seq_id} context_len {context_len} token_chunk_size {token_chunk_size}")
        ...
        get_tp_group().all_reduce(matched_tensor, op=torch.distributed.ReduceOp.MIN)
    ...
    def prefetch_kv_caches(
        ...
        if seq_group_metadata_list is not None:
            ...
            logger.info(f"broadcast_object_list num_prefill_requests {num_prefill_requests}")
