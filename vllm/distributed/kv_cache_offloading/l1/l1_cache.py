...
class L1Cache(MeasurableBase):
    ...
    def __repr__(self) -> str:
        return (f"L1Cache(policy={self._eviction_policy.name}, "
    ...
    def exists(
        ...
        for key in self._cache_block_keys(prefix, tokens):
            if key in self._eviction_policy:
                ...
                break
        return (Status(
            value=total) if total > 0 else Status(StatusCodes.NOT_FOUND))
    ...
    def put(
        ...
        prefix: Iterable[int] | None,
        tokens: Iterable[int],
        kv_tensors: torch.Tensor | Iterable[torch.Tensor]
    ...
    def _put_tensors_impl(
        ...
        Args:
            prefix (Iterable[int] | None): The prefix tokens of the kv tensors.
            tokens (Iterable[int]): The tokens of the kv tensors.
            kv_tensors (torch.Tensor | Iterable[torch.Tensor]): The kv tensors.
        Returns:
            The status of the put operation and the number of blocks.
        """
        if prefix is not None and len(prefix) % self.block_ntokens != 0:
        ...
        if isinstance(kv_tensors, torch.Tensor):
            ...
            if kv_tensors[0].shape != self.block_shape:
                return Status(
                    StatusCodes.INVALID,
                    (f"Key tensors shape {kv_tensors.shape} is not equal to "
        ...
        if isinstance(kv_tensors, torch.Tensor):
            ...
            for i in range(num_blocks):
                ...
                kv_blocks[i] = kv_tensors[tuple(slices)]
                offset += self.block_ntokens
        else:
            kv_blocks = kv_tensors
        ...
        with cpu_perf_timer() as get_copy_dur_ms:
            for i, block_mr in enumerate(block_mrs):
                cached_tensors = block_mr.to_tensor(self.block_dtype,
                                                    block_mr_shape)
                cached_tensors.copy_(kv_blocks[i])
        log_every_n_seconds(
            logger,
            logging.INFO,
        ...
        status = self._put_mrs_impl(prefix,
                                    tokens,
                                    block_mrs,
                                    with_check=False)
        if not status.is_ok():
            ...
            [mr.ref_down() for mr in block_mrs[status.value:]]
        return status
