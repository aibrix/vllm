...
class Connector(Generic[K, V]):
    ...
    def create(
        ...
        if backend_name == "ROCKSDB":
            ...
            return RocksDBConnector.from_envs(retention_type, conn_id)
        else:
            raise ValueError(f"Unknown connector type: {backend_name}")

    @classmethod
    @abstractmethod
    def from_envs(cls, retention_type: KVCacheRetentionType, conn_id: str):
    ...
    @property
    @abstractmethod
    def retention_type(self) -> KVCacheRetentionType:
        raise NotImplementedError
