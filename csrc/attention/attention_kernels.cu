...
#pragma unroll
    ...
    #if 0
        if(head_idx == 0 && seq_idx == 0) {
          const cache_t * test_ptr = k_ptr + offset1 * BLOCK_SIZE * x + offset2;
          printf("[%d, %d, %d]: token_idx-%d, physical_block_offset %d, j-%d, offset1-%d, offset2-%d, addr-%p\n", blockIdx.x, blockIdx.y, threadIdx.x,token_idx, physical_block_offset, j, offset1, offset2,test_ptr);
        }
    #endif
...
#if 0
      if(head_idx == 0 && seq_idx == 0) {
        printf("[%d, %d, %d]: scale %f token_idx-%d, physical_block_offset %d, qk-%f\n", blockIdx.x, blockIdx.y, threadIdx.x, scale, token_idx, physical_block_offset, qk);
      }
#endif
...
#if 0
      if(head_idx == 0 && seq_idx == 0) {
        printf("[%d, %d, %d]: token_idx-%d, physical_block_offset %d, qk-%f\n", blockIdx.x, blockIdx.y, threadIdx.x, token_idx, physical_block_offset, qk);
      }
#endif
...
#pragma unroll
      for (int i = 0; i < NUM_ROWS_PER_THREAD; i++) {
        ...
        if (row_idx < HEAD_SIZE && lane % NUM_V_VECS_PER_ROW == 0) {
          accs[i] += src[row_idx];
          //if(seq_idx < 1)
          //  printf("[%d, %d, %d], qk_max %f: row_idx %d accs[%d]:%f\n", blockIdx.x, blockIdx.y, threadIdx.x, qk_max, row_idx, i, accs[i]);
        }
      }
    }
    __syncthreads();
  ...
  // Write the final output.
  if (warp_idx == 0) {
    //printf("[%d, %d, %d], seq_idx-%d, num_heads-%d, max_num_partitions-%d, HEAD_SIZE-%d, head_idx-%d, partition_idx-%d, NUM_ROWS_PER_THREAD-%d, NUM_V_VECS_PER_ROW-%d\n", blockIdx.x, blockIdx.y, threadIdx.x,seq_idx, num_heads, max_num_partitions, HEAD_SIZE, head_idx, partition_idx, NUM_ROWS_PER_THREAD, NUM_V_VECS_PER_ROW);
  ...
  if (kv_cache_dtype == "auto" && block_size == 16) {
    if (query.dtype() == at::ScalarType::Float) {
      dattention_launcher<float, float, vllm::Fp8KVCacheDataType::kAuto, 16>(
          ...
          num_kv_heads, scale, alibi_slopes, kv_scale);
    ...
    if (query.dtype() == at::ScalarType::Float) {
      dattention_launcher<float, float, vllm::Fp8KVCacheDataType::kAuto, 32>(
          output, exp_sums, max_logits, tmp_out, query, layer_idx, num_layers, max_seq_len,
          seq_lens, cache_row_mapping, cache_col_mapping,
          num_kv_heads, kv_scale, alibi_slopes, scale);
    } else if (query.dtype() == at::ScalarType::Half) {
